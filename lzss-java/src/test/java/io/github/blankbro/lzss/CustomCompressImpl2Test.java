...
@Slf4j
public class CustomCompressImpl2Test extends BaseTest {
    ...
    public void test202401151734() throws DecoderException, IOException {
        ...
        for (Map.Entry<String, CaseInfo> entry : caseList.entrySet()) {
            ...
            if (caseInfo.singleDataPackageByteLength == null) {
                log.info("=========>>> singleDataPackageByteLength is null, continue");
                continue;
            }
            if (caseInfo.noCompressionByteLength == null) {
                log.info("=========>>> noCompressionByteLength is null, continue");
                continue;
            }
            ...
            for (int i = 0; i < totalCount; i++) {
                ...
                byte[] encodeBytes = CustomCompressImpl2.encode(byteArr, caseInfo.noCompressionByteLength, caseInfo.singleDataPackageByteLength, bytePositionByteLength);
                long end = System.nanoTime();
                long encodeHandleTime = end - start;
                encodeTotalTime += encodeHandleTime;

                start = System.nanoTime();
                byte[] decodeBytes = CustomCompressImpl2.decode(encodeBytes, caseInfo.noCompressionByteLength, caseInfo.singleDataPackageByteLength, bytePositionByteLength);
                end = System.nanoTime();
